...
// V2.2 — an internal agent never connects to an inbox (it is a team copilot,
// never customer-facing). Show an informative state instead of the connect UI
// and skip the channels fetch entirely.
...
<template>
  <div
    v-if="isInternal"
    class="flex flex-col items-center justify-center w-full h-full max-w-md gap-3 px-6 py-12 mx-auto text-center"
  >
    <span
      class="flex items-center justify-center rounded-full size-12 bg-n-iris-3 text-n-iris-11"
    >
      <Icon icon="i-lucide-shield" />
    </span>
    <h2 class="text-sm font-medium text-n-slate-12">
      {{ t('AGENTS.CHANNELS.INTERNAL_TITLE') }}
    </h2>
    <p class="text-sm text-n-slate-11">
      {{ t('AGENTS.CHANNELS.INTERNAL_BODY') }}
    </p>
  </div>

  <div
    v-else
    class="flex flex-col w-full h-full max-w-3xl gap-6 px-6 py-6 mx-auto"
  >
    <div
