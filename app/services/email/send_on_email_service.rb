class Email::SendOnEmailService < Base::SendOnChannelService
  ...
  def perform_reply
    ...
    # Microsoft: envia via Graph /sendMail (imune ao Security Defaults que bloqueia SMTP AUTH).
    # Demais provedores: caminho SMTP de sempre (intocado).
  ...
  # Cadeia completa de References (mesma lógica do SMTP, via ReferencesHeaderBuilder)
  # para preservar o threading em conversas longas; cai no In-Reply-To se não houver cadeia.
  def references_header
    build_references_header(conversation, in_reply_to).presence || in_reply_to
  end
end
