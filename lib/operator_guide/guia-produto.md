...
- rota: `contacts_dashboard_index` - `/app/accounts/:accountId/contacts`
- gate: feature flag `crm`; permissões `administrator`, `agent`, `contact_manage`
...
- rota: `profile_settings_index` - `/app/accounts/:accountId/profile/settings`
- gate: papeis `administrator`, `agent`, `custom_role`; secao oculta quando a instalacao define `disableUserProfileUpdate`
...
- gate: feature `campaigns`; permissão `administrator`
...
- gate: feature `campaigns`; feature flag `whatsapp_campaigns`; permissão `administrator`
...
- rota: `crm_calendar_index` - `/app/accounts/:accountId/crm/calendar`
- gate: `CRM_KANBAN_ENABLED=true`; o chip Reuniões depende de `CRM_CALENDAR_MEETINGS_ENABLED=true`; o chip Externos depende de calendário conectado à caixa.
...
- rota: `settings_inbox_new` - `/app/accounts/:accountId/settings/inboxes/new`
- gate: papel `administrator` para reconectar/reautorizar
- diagnostic: `channel`
...
- rota: `profile_settings_index` - `/app/accounts/:accountId/profile/settings`
- gate: qualquer perfil (são preferências do próprio usuário)
...
- rota: `autonomia_agents_index` - `/app/accounts/:accountId/autonomia/agents`
- gate: papel `administrator`
- diagnostic: `ai_agent`
