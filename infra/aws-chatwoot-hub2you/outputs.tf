...
output "ecr_repository_name" {
  value = aws_ecr_repository.chatwoot.name
}
...
output "dns_instruction" {
  value = "Create a CNAME outside AWS: ${var.domain_name} -> ${aws_lb.ec2.dns_name}."
}
...
output "ssm_runtime_image_parameter" {
  value = aws_ssm_parameter.chatwoot_ec2_runtime_image.name
}
